...
function Accordion() {
  ...
  const toggleMode = () => {
    ...
    if (singleMode) {
      ...
      // При переходе в режим "один открытый" оставляем только первый открытый элемент
  ...
  return (
    <div className="accordion">
      ...
      {faqItems.map((item, index) => (
        <div 
          key={index}
          className={`accordion-item ${activeItems.includes(index) ? 'active' : ''}`}
        >
          <div 
          ...
          <div 
            ...
            style={{
              maxHeight: activeItems.includes(index) 
                ? document.querySelector(`.accordion-item:nth-child(${index + 2}) .accordion-content`)?.scrollHeight + 'px' 
                : '0'
            }}
          >
